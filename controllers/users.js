...
module.exports.createUser = async (
  ...
  try {
    ...
    res
      .status(201)
      .json({ userData: convertDataForClient(newUser.toObject()), token });
...
module.exports.authUser = async ({ body: { userName, password } }, res) => {
  try {
    ...
    res.json({
      userData: convertDataForClient(user.toObject(), 'user'),
      token,
    });
  } catch (error) {
    errorHandler(res, error);
...
module.exports.isValid = async (req, res) => {
  try {
    ...
    if (!user) {
      return res
        .status(400)
        .json({ message: 'Извините, но такого пользователя уже нет' });
    }
    if (user.boughtGoods.length) {
      await createPopuldatedData(user, 'boughtGoods');
    }
...
module.exports.getUserCart = async (req, res) => {
  try {
    const user = await Users.findById(req.user.userId),
      userCartData = await createPopuldatedData(
        user,
        'cart.cartItems.commodityId',
      ),
      userCart = userCartData.cart.cartItems.map(item => {
        const id = item.commodityId._id,
          ...
          category = item.commodityId.category,
...
module.exports.updateUserData = async (
  { body, user: { userId }, file },
  res,
) => {
  try {
    const newAvatar = {},
    ...
    const oldUserData = await Users.findByIdAndUpdate(userId, {
      ...body,
      ...newAvatar,
      ...newPassword,
    });
    res
      .status(200)
      .json({ message: `Данные пользователя с id: ${userId} обновлены` });
    if (
      newAvatar?.avatar &&
      getValidFileName(oldUserData.avatar) !== 'defaultAvatar.png'
    ) {
