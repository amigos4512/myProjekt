...
const createUser = (userData, token) =>
  createAction(CREATE_NEW_USER, { ...userData, token });
...
export const updateBoughtGoods = boughtGoods =>
  createAction(UPDATE_BOUGHTGOODS, boughtGoods);

export const userLogin = (userData, token) =>
  createAction(LOGIN, { ...userData, token });

export const userLogout = () => createAction(LOGOUT);

export const invalidRoute = () => createAction(INVALID_ROUTE);

export const updateUserReviews = dataForUpdate =>
  createAction(UPDATE_USER_REVIEWS, dataForUpdate);
...
export const authorization = (data, formData, history) => async dispatch => {
  try {
    ...
    dispatch(
      userLogin(
        { ...userData, avatar: null, avatarSrc: userData.avatar },
        token
      )
    );
    ...
    const goods = await GoodsService.getBestGoods();
