...
const Slider = ({
  ...
  const nextBtnClick = () => {
    const slideLeft = Math.round(
      content.length -
        (Math.abs(position) + slidesToShow * slideWidth) / slideWidth,
    );
    const newPosition =
      slideLeft >= slidesToScroll
        ? position - movePosition
        : position - slideLeft * slideWidth;
    if (slideLeft === 0) {
      setPosition(0);
    } else {
  ...
  const setDotClass = dotIndex => {
    if (Math.abs(position) === dotIndex * slideWidth) {
      return (
        classNames.sliderDotsItemActive ??
        'sliderDots__item sliderDots__item_active'
      );
    } else {
