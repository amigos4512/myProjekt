...
const updateImgData = (newData, oldData, type) => {
  return {
    [`${type}File`]: !newData[`${type}File`]
      ? oldData[`${type}File`]
      : newData[`${type}File`],
    [`${type}Src`]: !newData[`${type}Src`]
      ? oldData[`${type}Src`]
      : newData[`${type}Src`],
    [`${type}Alt`]: createAlt(newData, oldData, type),
    [`${type}Id`]:
      !oldData || !oldData[`${type}Id`]
        ? StringHelper.createId()
        : oldData[`${type}Id`],
  };
};
...
const updateField = (commodityData, fieldName, newFieldData) => {
  if (fieldName === 'img' || fieldName === 'previewImg') {
    const newImgData = updateImgData(
      newFieldData,
      commodityData[fieldName],
      fieldName,
    );
    return {
      ...commodityData,
...
const updateReviewsData = (commodityData, newReviewData, type = 'update') => {
  const { reviews, countReviews } = commodityData,
    reviewIndex = reviews.findIndex(
      review => review.reviewId === newReviewData.reviewId,
    ),
    newData = {};
  ...
  if (reviewIndex === -1) {
    ...
    newData.reviews = changeArrayElement(reviews, reviewIndex, {
      ...reviews[reviewIndex],
      ...newReviewData,
    });
...
const updateRating = ({ rating }, { newRating, oldRating }) => {
  ...
  newRatingData.general =
    sumOfRatings && numberOfRatings
      ? (sumOfRatings / numberOfRatings).toFixed(1)
      : 0;
  ...
  switch (action.type) {
    ...
    case UPDATE_USER_REVIEW:
      return {
        ...state.commodityData,
        userReview: {
          ...state.commodityData.userReview,
          ...action.payload,
          reviewWasRemoved: false,
        },
      };
    ...
    case UPDATE_CATEGORY:
      return updateField(state.commodityData, 'category', action.payload);
